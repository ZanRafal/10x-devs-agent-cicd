...
const PACKAGE_NAME = "@zanrafal/ai-toolkit";
...
function copyDir(source, target, installedFiles, root) {
  ...
  for (const entry of fs.readdirSync(source, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      ...
      installedFiles.push(path.relative(root, dst).split(path.sep).join("/"));
    }
  }
}
...
function main() {
  ...
  console.log(`${PACKAGE_NAME}@${PACKAGE_VERSION}: installed ${installedFiles.length} file(s)`);
}
